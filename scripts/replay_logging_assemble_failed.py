"""
    ...
    But it is for assemble tasks only, not for disassemble tasks.
"""
...
def parser_args():
    ...
    parser.add_argument('--task_name', type=str, default='ONE_PEG_IN_HOLE', help='Name of the task to run')
    ...
    parser.add_argument('--use_hand_back_camera',type=bool, default=False, help='Whether to use hand back camera')
    args = parser.parse_args()
    # select assets path, convenient for running in different environments
    print("\033[94mChoose a way to load assets:\033[0m")         
...
def main(args=None):
    ...
    my_env = FrankaAssemblyGym(is_replay=True,
    ...
    for i in range(len(data_file_paths)):
        ...
        if my_env.task.support['need']:
            for name, obj in my_env.task.objects_support.items():
                position = data_logger.get_data_frame(data_frame_index=0).data['support'][name]['position']
                rotation = data_logger.get_data_frame(data_frame_index=0).data['support'][name]['orientation']
                obj.set_world_pose(position=position, orientation=rotation)
        ...
        while simulation_app.is_running():
            ...
            if my_env.world.is_playing():
                if my_env.world.current_time_step_index <= data_logger.get_data_frame(data_logger.get_num_of_data_frames()-1).current_time_step:
                    ...
                    break
        # create demo group
        demo_group = group.create_group(f"demo_{all_demo_count}")
        all_demo_count += 1
        ...
        obs_demo_group.create_dataset("ee_poses", data=ee_poses)
        if use_depth:
    ...
    print('Total demo count: ',all_demo_count," success_demo_count: ",success_demo_count, " Failed files: ",failed_files)
    hdf5_file.close()
