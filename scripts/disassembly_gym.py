...
from .disassembly_task import DisassemblyTask
from InduMan.utils import get_default_data_path, get_assets_path, ForceMonitor

class FrankaDisassemblyGym(gym.Env):
    def __init__(self,
                 save_path=None,
                 render=True,
                 is_replay=False,
        ...
        self.force_monitor = ForceMonitor(threshold=3, min_interval=1)
        ...
        self.data_path = os.path.join(self.save_path, task_name)
        
        self.world.scene.add_default_ground_plane()
        
        # add task
        task_assets_path = self.assets_root_path if task_assets_path is None else task_assets_path
        self.task = DisassemblyTask(name = self.task_name,
    ...
    def add_shpere_light(self,radius=0.25,color=np.array([1.0,1.0,1.0]),
        ...
        light_prim.AddTranslateOp().Set(Gf.Vec3f(position[0],position[1],position[2]))  # set position  
    ...
    def step(self, action):
        print(f"action: {action}","data type: ",action.dtype)
